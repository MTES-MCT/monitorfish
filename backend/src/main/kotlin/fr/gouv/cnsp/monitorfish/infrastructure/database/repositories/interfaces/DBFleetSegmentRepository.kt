...
interface DBFleetSegmentRepository : CrudRepository<FleetSegmentEntity, Long> {
    ...
    @Modifying(clearAutomatically = true)
    @Query(
        value = "UPDATE fleet_segments SET gears = cast(:gears AS varchar[]) WHERE segment = :segment",
        nativeQuery = true
    )
    ...
    @Modifying(clearAutomatically = true)
    @Query(
        value = "UPDATE fleet_segments SET fao_areas = cast(:faoAreas AS varchar[]) WHERE segment = :segment",
        nativeQuery = true
    )
    ...
    @Modifying(clearAutomatically = true)
    @Query(
        value = "UPDATE fleet_segments SET target_species = cast(:targetSpecies AS varchar[]) WHERE segment = :segment",
        nativeQuery = true
    )
    ...
    @Modifying(clearAutomatically = true)
    @Query(
        value = "UPDATE fleet_segments SET bycatch_species = cast(:bycatchSpecies AS varchar[]) WHERE segment = :segment",
        nativeQuery = true
    )
    ...
    @Modifying(clearAutomatically = true)
    @Query(
        value = "UPDATE fleet_segments SET impact_risk_factor = :impactRiskFactor WHERE segment = :segment",
        nativeQuery = true
    )
