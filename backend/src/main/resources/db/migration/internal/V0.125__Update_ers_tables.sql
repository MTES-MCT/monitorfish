...
ALTER TABLE ers
    RENAME TO logbook_reports;
ALTER TABLE ers_messages
    RENAME TO logbook_raw_messages;
...
-- An index is already created with the primary key, so this one can be dropped
...
ALTER TABLE logbook_reports
    ALTER COLUMN operation_number TYPE VARCHAR(100);
ALTER TABLE logbook_reports
    ALTER COLUMN report_id TYPE VARCHAR(100);
ALTER TABLE logbook_reports
    ALTER COLUMN referenced_report_id TYPE VARCHAR(100);
ALTER TABLE logbook_raw_messages
    ALTER column operation_number TYPE VARCHAR(100);

-- Adjust log_type VARCHAR max length to accomodate for FLUX's new log types
ALTER TABLE logbook_reports
    ALTER COLUMN log_type TYPE VARCHAR(100);
