CREATE OR REPLACE FUNCTION find_all_enriched_pno_references_and_related_operations(
    ...
    createdBefore TIMESTAMP WITH TIME ZONE DEFAULT NULL,
)
RETURNS TABLE (
    id bigint,
        ...
        filtered_pnos AS (
            ...
            WHERE
                ...
                AND (
                    isZero IS NULL 
                    OR (
                        EXISTS (SELECT 1 FROM jsonb_array_elements(lr.value->'catchOnboard')) 
                        AND NOT EXISTS (
                            SELECT 1 
                            FROM jsonb_array_elements(lr.value->'catchOnboard') AS catchOnboard
                            WHERE COALESCE((catchOnboard->>'weight')::DOUBLE PRECISION, 0) != 0
                        )
                    )
                )
                AND (createdBefore IS NULL OR lr.report_datetime_utc <= createdBefore)
        ),
