...
CREATE FUNCTION find_all_enriched_pno_references_and_related_operations(
...
RETURNS TABLE (
    ...
    BEGIN
        ...
        distinct_cfrs AS MATERIALIZED (
            SELECT DISTINCT pnos.cfr
            FROM pnos
        ),

        cfr_reporting_counts AS MATERIALIZED (
            SELECT
                dc.cfr,
        ...
        pnos_and_reporting_count AS MATERIALIZED (
            SELECT
                dacplrwecarc.*,
        ...
        acknowledged_report_ids AS MATERIALIZED (
