...
class GetPriorNotificationsITests : AbstractDBTests() {
    ...
    @Test
    @Transactional
    fun `execute should return a list of prior notifications sorted by prior notification types ascending`() {
        // Given
        val sortColumn = LogbookReportSortColumn.PRIOR_NOTIFICATION_TYPES
        val sortDirection = Sort.Direction.ASC
        ...
        // Then
        val (priorNotifications, totalLength) = result
        val firstPriorNotificationWithPriorNotificationTyoes = priorNotifications
            .first { it.logbookMessageTyped.typedMessage.pnoTypes.isNotEmpty() }
        assertThat(firstPriorNotificationWithPriorNotificationTyoes.logbookMessageTyped.typedMessage.pnoTypes[0].name)
            .isEqualTo("Espèces soumises à préavis")
        assertThat(priorNotifications).hasSize(10)
        assertThat(totalLength).isEqualTo(14)
    }
    ...
    @Test
    @Transactional
    fun `execute should return a list of prior notifications sorted by prior notification types descending`() {
        // Given
        val sortColumn = LogbookReportSortColumn.PRIOR_NOTIFICATION_TYPES
        val sortDirection = Sort.Direction.DESC
    ...
    @Test
    @Transactional
    fun `execute should return a list of prior notifications sorted by trip segment codes ascending`() {
        // Given
        val sortColumn = LogbookReportSortColumn.TRIP_SEGMENT_CODES
        val sortDirection = Sort.Direction.ASC
        ...
        // Then
        val (priorNotifications, totalLength) = result
        val firstPriorNotificationWithSegmentCodes = priorNotifications
            .first { it.logbookMessageTyped.logbookMessage.tripSegments?.isNotEmpty() == true }
        assertThat(firstPriorNotificationWithSegmentCodes.logbookMessageTyped.logbookMessage.tripSegments!![0].code)
            .isEqualTo("FR_SCE")
        assertThat(priorNotifications).hasSize(10)
        assertThat(totalLength).isEqualTo(14)
    }
    ...
    @Test
    @Transactional
    fun `execute should return a list of prior notifications sorted by trip segment codes descending`() {
        // Given
        val sortColumn = LogbookReportSortColumn.TRIP_SEGMENT_CODES
        val sortDirection = Sort.Direction.DESC
