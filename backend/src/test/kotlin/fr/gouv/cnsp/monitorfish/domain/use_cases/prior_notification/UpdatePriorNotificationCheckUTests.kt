...
class UpdatePriorNotificationCheckUTests {
    @MockBean
    private lateinit var priorNotificationCheckRepository: PriorNotificationCheckRepository

    @Test
    fun `execute Should return the saved prior notification check`() {
        val fakePriorNotificationCheck = PriorNotificationCheck.new("FAKE_REPORT_ID")

        // Given
        given(priorNotificationCheckRepository.save(any())).willReturn(fakePriorNotificationCheck)

        // When
        val result = UpdatePriorNotificationCheck(priorNotificationCheckRepository).execute(fakePriorNotificationCheck)
