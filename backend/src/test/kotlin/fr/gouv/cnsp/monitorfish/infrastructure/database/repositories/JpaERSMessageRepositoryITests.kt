...
class JpaERSMessageRepositoryITests : AbstractDBTests() {

    @Autowired
    private lateinit var jpaERSMessageRepository: JpaERSMessageRepository
    ...
    @BeforeEach
    fun setup() {
        cacheManager.getCache("ers_raw_message")?.clear()
    }
    ...
    fun `findRawMessage Should return the raw message`() {
        // When
        val rawMessage = jpaERSMessageRepository.findRawMessage("OOF20190126059903")

        // Then
        assertThat(rawMessage).isEqualTo("<ers:OPS AD=\"FRA\" FR=\"OOE\" ON=\"OOF20190126059903\" OD=\"2021-01-28\" OT=\"23:53\" EVL=\"TurboCatch (3.5-5)\">\n" +
                "<ers:DAT TM=\"CU\">\n" +
    ...
    fun `findRawMessage Should throw an exception When the message is not found`() {
        // When
        val throwable = catchThrowable { jpaERSMessageRepository.findRawMessage("BAD_OP") }
