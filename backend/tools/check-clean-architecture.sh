...
readonly UNAUTHORIZED_PACKAGES_USAGE_COUNT_IN_USE_CASES=$(find ${USE_CASES_PACKAGE_PATH} \( -name "*.kt" -o -name "*.java" \) -exec egrep -w ${UNAUTHORIZED_PACKAGES_USAGE_IN_USE_CASES} {} \; | wc -l)
...
find ${USE_CASES_PACKAGE_PATH} \( -name "*.kt" -o -name "*.java" \) -exec egrep -lw ${UNAUTHORIZED_PACKAGES_USAGE_IN_USE_CASES} {} \;
exit 1
