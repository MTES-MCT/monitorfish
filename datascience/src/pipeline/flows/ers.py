...
from sqlalchemy import Table, MetaData, Boolean, Date, Float, Integer, String, Date, DateTime
...
RAW_XML_TABLE = {
    "dam": 'raw_jpe_dam_xml',
    "dpma": 'raw_jpe_dpma_xml'
}

PROCESSED_XML_TABLE = {
    "dam": 'processed_jpe_dam_xml',
    "dpma": 'processed_jpe_dpma_xml'
}
    

@task
def extract_raw_xml(data_source: str = "dam", 
                    chunksize: Union[None, int]= 10000) -> Iterator[pd.DataFrame]:
...
def parse_xml(raw_xml: Iterator[pd.DataFrame]) -> Iterator[pd.DataFrame]:
    ...
    for raw_xml_chunk in raw_xml:
        ...
        log_cols = ["far", "dep", "rlc", "tra", "coe", "cox", "cro", "trz", 
