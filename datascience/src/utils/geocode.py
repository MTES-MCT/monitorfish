...
def geocode(query_string=None, country_code_iso2=None, **kwargs):
    ...
    params = {
        "key": os.environ["LOCATIONIQ_TOKEN"],
        "format": "json"
    }
    
    if query_string is not None and not pd.isna(query_string):
        if kwargs:
            print("Keyword arguments cannot be used in combination with text query. Keyword arguments will be ignored.")
        ...
        print("You must provide either a query string or at least 1 of the allowed keyword arguments.")
        return None, None
