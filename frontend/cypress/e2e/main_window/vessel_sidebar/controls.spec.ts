...
context('Vessel sidebar controls tab', () => {
  ...
  it('Controls Should contain the controls summary and history', () => {
    ...
    // When
    cy.get(`[data-cy="vessel-control-years"] > li[title="Année ${currentYear}"]`).click({ timeout: 10000 })
    ...
    cy.get(`[data-cy="vessel-control-years"] > li[title="Année ${currentYear}"] button:first-of-type`).first().click({ timeout: 10000 })

    // Check the order of controls (in descending order)
    if (currentMonth === 0) {
      cy.get(`[data-cy="vessel-control-years"] > li[title="Année ${currentYear - 1}"]`).click({ timeout: 10000 })
      ...
      cy.get(`[data-cy="vessel-control-years"] > li[title="Année ${currentYear - 2}"]`).click({ timeout: 10000 })
