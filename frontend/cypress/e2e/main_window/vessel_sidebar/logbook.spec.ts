...
context('Vessel sidebar logbook tab', () => {
  ...
  it('Fishing Should contain the vessel ERS logbook messages', () => {
    ...
    cy.get('*[data-cy="lan-message-resume"]').contains('BONITE A DOS RAYE (BON)')
    // Contains species not in the LAN but in the PNO
    cy.get('*[data-cy="lan-message-resume"]').contains('SOLE COMMUNE (SOL)')
  ...
  it('Fishing trips Should be walkable', () => {
    ...
    // Then
    cy.get('*[data-cy^="vessel-fishing-previous-trip"]').click({ timeout: 10000 })
    cy.get('input[name="tripNumber"]').should('have.value', '20230087')

    cy.get('*[data-cy^="vessel-fishing-previous-trip"]').click({ timeout: 10000 })
    cy.get('input[name="tripNumber"]').should('have.value', '20230086')
