...
// Circle computed around the stubbed sea control coordinates, rounded to 6 decimals (~10 cm).
const EXPECTED_MISSION_ZONE_COORDINATES = [
  [-3.872, 47.101986],
  [-3.86941, 47.1019],
  [-3.866845, 47.101641],
  [-3.86433, 47.101212],
...
context('Side Window > Mission Form > Sea Control', () => {
  ...
  it('Should fill the mission zone from the last sea control added', () => {
    ...
    cy.waitForLastRequest(
      ...
      const [firstPolygon, ...otherPolygons] = interception.request.body.geom.coordinates
      expect(otherPolygons).to.have.length(0)

      // The circle is built from trigonometry, whose last digits shift between browser engine
      // versions, so coordinates are rounded to ~10 cm before being compared.
      const zoneCoordinates = firstPolygon[0].map(([longitude, latitude]) => [
        Number(longitude.toFixed(6)),
        Number(latitude.toFixed(6))
      ])

      expect(zoneCoordinates).to.deep.equal(EXPECTED_MISSION_ZONE_COORDINATES)
    })
  })
