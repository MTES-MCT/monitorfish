...
context('Side Window > Mission List > Export Activity Reports', () => {
  ...
  it('Should download an activity report', () => {
    ...
    cy.fill('Début', [startYear, startMonth, startDay])
    ...
    cy.fill('Fin', [endYear, endMonth, endDay])
    // Hack to close the date picker popupœ
    cy.get('h4').contains('Exporter les ACT-REP').click().wait(250)
    cy.fill('JDP', 'JDP NS-01')

    console.info(
      `/bff/v1/mission_actions/controls/activity_reports?beforeDateTime=${endYear}-${endMonth}-${endDay}T23:59:59.000Z&afterDateTime=${startYear}-${startMonth}-${startDay}T00:00:00.000Z&jdp=NORTH_SEA`
    )
    cy.intercept(
      'GET',
    ...
    cy.getDownloadedFileContent(content => {
      content
        ...
        .should('contains', '"LCross Etel","L","Cross Etel","","INSPECTION",')
        .should('contains', '"FRA","FRA","","","","Vessel","FRA","DONTSINK"')
        .should(
          'contains',
