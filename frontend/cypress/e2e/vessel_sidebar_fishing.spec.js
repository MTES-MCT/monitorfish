...
context('Vessel sidebar fishing tab', () => {
  ...
  it('Fishing Should contain the vessel fishing resume', () => {
    ...
    // Then
    cy.get('*[data-cy^="vessel-fishing-gears"]', { timeout: 10000 }).should('be.visible')
    cy.get('*[data-cy^="vessel-fishing-gears"]').contains('Trémails et filets maillants combinés (GTN)', { timeout: 10000 })
    cy.get('*[data-cy^="vessel-fishing-resume-title"]').contains('1 message - 2256 kg pêchés au total', { timeout: 10000 })
  })
  ...
  it('Fishing Should contain the vessel ERS logbook messages', () => {
    ...
    cy.get('*[data-cy="vessel-fishing-message"]').first().contains('Al Jazeera Port le 11/10/2019 à 01h40 (UTC)', { timeout: 10000 })
  ...
  it('Fishing Should contain the vessel FLUX logbook messages', () => {
    ...
    cy.get('*[data-cy="vessel-fishing-message"]').first().contains('Départ', { timeout: 10000 })
    cy.get('*[data-cy="vessel-fishing-message"]').first().contains('ESCAR le 06/05/2020 à 11h39 (UTC)', { timeout: 10000 })
