...
context('Vessels Track', () => {
  ...
  it('Last position card with Beacon malfunction Should be seen on the map on pointer move', () => {
    ...
    cy.get('.vessels').trigger('pointermove', { clientX: 910, clientY: 780, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 982, clientY: 787, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 980, clientY: 785, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 979, clientY: 785, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 985, clientY: 787, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 985, clientY: 787, force: true, pointerId: 1 })
    cy.get('.vessels').trigger('pointermove', { clientX: 984, clientY: 785, force: true, pointerId: 1 })
  ...
  it('A track Should be showed When clicking on a vessel with CTRL key pressed', () => {
    ...
    cy.wait(200)
    cy.get('.vessels').click(295, 297, { force: true, timeout: 10000 })
  ...
  it('A track Should be showed When clicking on a vessel with the custom map menu', () => {
    ...
    cy.get('.vessels').rightclick(504, 289, { force: true, timeout: 10000 })
    cy.get('*[data-cy^="show-vessel-tracks-menu-options"]').click({ force: true })
