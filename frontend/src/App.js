...
function App() {
  ...
  switch (browserName) {
    ...
    case 'Edge':
      if (browserVersionAsNumber < 79) {
        return getUnsupportedBrowser()
      }
      break
    case 'Chrome':
      if (browserVersionAsNumber < 69) {
        return getUnsupportedBrowser()
      }
      break
    case 'Firefox':
      if (browserVersionAsNumber < 62) {
        return getUnsupportedBrowser()
      }
      break
    case 'Safari':
      if (browserVersionAsNumber < 12) {
        return getUnsupportedBrowser()
      }
      break
    case 'Opera':
      if (browserVersionAsNumber < 56) {
        return getUnsupportedBrowser()
      }
      break
  }
...
function HomePage() {
  ...
  return (
    <>
      <StateManager adminRole inBackofficeMode={false} />
      <Switch>
        <Route exact path="/side_window">
          <SideWindow fromTab openedSideWindowTab={sideWindowMenu.ALERTS.code} />
        </Route>
        <Route exact path="/">
          <Healthcheck />
...
function TritonFish() {
  ...
  return (
    <>
      <StateManager adminRole={false} inBackofficeMode={false} />
      <Healthcheck />
      <PreviewFilteredVessels />
      <Wrapper>
        <Map />
