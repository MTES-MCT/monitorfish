...
function getAllGeometryWithoutProperty(fromBackoffice) {
  ...
  const REQUEST =
    `${geoserverURL}/geoserver/wfs?service=WFS&version=1.1.0&request=GetFeature&typename=monitorfish:` +
    `${Layers.REGULATORY.code}&outputFormat=application/json&propertyName=geometry,id&CQL_FILTER=` +
  ...
  try {
    ...
    return fetch(
      `${geoserverURL}/geoserver/wfs?service=WFS` +
        ...
        '&propertyName=law_type,topic,gears,species,regulatory_references,zone,facade,region'
          .replace(/'/g, '%27')
          .replace(/\(/g, '%28')
          .replace(/\)/g, '%29')
          .replace(/ /g, '%20')
