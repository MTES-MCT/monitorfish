...
const updateBeaconMalfunctionFromKanban = (id, nextBeaconMalfunction, updatedFields) => (dispatch, getState) => {
  ...
  const beaconMalfunctionToUpdateIsOpenedAsCurrentVesselMalfunction =
    getState().beaconMalfunction.vesselBeaconMalfunctionsResumeAndHistory?.current?.beaconMalfunction?.id === id
  const beaconMalfunctionToUpdateIsOpened =
    getState().beaconMalfunction.openedBeaconMalfunction?.beaconMalfunction?.id === id
  const beaconMalfunctionToUpdateIsOpenedInKanban =
    getState().beaconMalfunction.openedBeaconMalfunctionInKanban?.beaconMalfunction?.id === id

  return updateBeaconMalfunctionFromAPI(id, updatedFields)
    .then(updatedBeaconMalfunctionWithDetails => {
