...
export const showVesselTrack =
  (vesselIdentity: VesselIdentity, isCalledFromCron: boolean, trackRequest: TrackRequest | null, hasZoom?: boolean) =>
  async (dispatch, getState) => {
    try {
      ...
      dispatch(doNotAnimate(isCalledFromCron))
      dispatch(removeError())

      const { isTrackDepthModified, positions } = await getVesselPositionsFromAPI(vesselIdentity, nextTrackRequest)
      try {
        throwCustomErrorFromAPIFeedback(positions, isTrackDepthModified, isCalledFromCron)
        dispatch(removeError())
      } catch (error) {
