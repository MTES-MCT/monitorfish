...
export const getAdministrativeZones =
  ...
  async (dispatch, getState): Promise<GroupedZonesAndZones> => {
    ...
    const groupedZonesToFetch: Promise<GroupAndZones>[] = Object.values(AdminLayerProperties)
      .filter(zone => zone.hasFetchableZones)
      .map(async zone => {
        ...
        const nextZones: MonitorFishMap.AdminShowableLayer[] = fetchedZones.features.map(feature => ({
