...
const RegulatoryText = props => {
  const { setRegulatoryText, id, regulatoryText, addOrRemoveRegulatoryTextInList, listLength, saveForm } = props

  const { reference, url, startDate, endDate, textType } = regulatoryText
      ...
      <ContentLine>
        ...
        {isEditing ? (
          <>
            ...
            {(reference || url) && (
              <>
                <ValidateButton disabled={false} isLast={false} onClick={checkNameAndUrl} data-cy="save-reg-text-name">
                  Enregistrer
                </ValidateButton>
                <CancelButton
                  disabled={false}
          ...
          <Tag tagValue={reference} tagUrl={url} onCloseIconClicked={onCloseIconClicked} />
        )}
      </ContentLine>
      <ContentLine>
