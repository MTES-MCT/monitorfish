...
export function GearRegulationDisplayed() {
  ...
  const { otherInfo, authorized, unauthorized } = gearRegulation
      ...
      {gearRegulationIsNotEmpty ? (
        <Section>
          {hasAuthorizedContent ? (
            <RegulatedGears authorized={true} regulatedGearsObject={authorized || DEFAULT_AUTHORIZED_REGULATED_GEARS} />
          ) : null}
