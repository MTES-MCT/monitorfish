...
export const renderVesselAlertFeatures = (): MainAppThunk => async (_, getState) => {
  const isLayerFound = monitorfishMap
    .getLayers()
    .getArray()
    ?.find(layer => layer.get('code') === MonitorFishMap.MonitorFishLayer.VESSEL_ALERT)
  if (!isLayerFound) {
    return
  }
  ...
  const features = vesselsFeatures
    ...
    .map(feature => {
      ...
      nextFeature.setId(`${LayerProperties.VESSEL_ALERT.code}:${getVesselCompositeIdentifier(feature.getProperties())}`)
