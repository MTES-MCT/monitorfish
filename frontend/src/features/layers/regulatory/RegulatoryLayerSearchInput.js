...
function orderByAlphabeticalLayer (foundRegulatoryZones) {
...
const RegulatoryLayerSearchInput = props => {
  ...
  useEffect(() => {
    ...
    Object.keys(searchFields).forEach(searchProperty => {
      if (searchFields[searchProperty].searchText.length > 0) {
        let searchFieldFoundRegulatoryLayers
        if (searchFields[searchProperty].properties === searchFields.gearSearchText.properties) {
          searchFieldFoundRegulatoryLayers = searchGears(
            searchFields[searchProperty].searchText,
            regulatoryLayers)
        } else {
          searchFieldFoundRegulatoryLayers = search(
            searchFields[searchProperty].searchText,
        ...
        if (foundRegulatoryLayers && Object.keys(foundRegulatoryLayers).length === 0) {
          foundRegulatoryLayers = searchFieldFoundRegulatoryLayers
        } else if (foundRegulatoryLayers) {
          foundRegulatoryLayers = getMergedRegulatoryLayers(foundRegulatoryLayers, searchFieldFoundRegulatoryLayers)
        }
      }
    })
  ...
  function getMergedRegulatoryLayers (foundRegulatoryLayers, searchFieldFoundRegulatoryLayers) {
    const mergedRegulatoryLayers = {}

    Object.keys(foundRegulatoryLayers).forEach(regulatoryLayer => {
      foundRegulatoryLayers[regulatoryLayer].forEach(layerZone => {
        if (searchFieldFoundRegulatoryLayers[regulatoryLayer] &&
          searchFieldFoundRegulatoryLayers[regulatoryLayer].length &&
          searchFieldFoundRegulatoryLayers[regulatoryLayer].some(searchSubZone =>
            searchSubZone.layerName === layerZone.layerName &&
            searchSubZone.zone === layerZone.zone
          )) {
          mergedRegulatoryLayers[regulatoryLayer] = mergedRegulatoryLayers[regulatoryLayer]
            ? mergedRegulatoryLayers[regulatoryLayer].concat(layerZone)
            : [].concat(layerZone)
        }
      })
    })
