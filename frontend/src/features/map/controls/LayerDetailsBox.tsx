...
export function LayerDetailsBox({ feature }: LayerDetailsBoxProps) {
  ...
  const vectorLayerStyle = useMemo(
    () =>
      getAdministrativeAndRegulatoryLayersStyle(Layers.REGULATORY.code)(
        undefined,
        regulatoryFeatureProperties as BaseRegulatoryZone
      ),
...
const Rectangle = styled.div<{
  ...
  height: 14px;
  background: ${p =>
    p.vectorLayerStyle?.getFill() ? p.vectorLayerStyle.getFill().getColor()?.toString() : COLORS.gray};
  border: 1px solid
    ${p =>
      p.vectorLayerStyle?.getStroke() ? p.vectorLayerStyle.getStroke().getColor().toString() : COLORS.grayDarkerTwo};
  margin-right: 7px;
  margin-top: 5px;
`
