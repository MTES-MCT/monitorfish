...
function VesselList({ namespace }) {
  const dispatch = useDispatch()
  const { previewFilteredVesselsMode, rightMenuIsOpen, vesselListModalIsOpen } = useSelector(state => state.global)
  const {
  ...
  return (
    <>
      <Wrapper isFiltering={isFiltering}>
        ...
        <Modal backdrop="static" onClose={() => closeAndResetVesselList()} open={vesselListModalIsOpen} size="full">
          <StyledModalHeader isFull>
            <Modal.Title>
              <VesselIcon $background={COLORS.charcoal} $isTitle /> Liste des navires avec VMS
            </Modal.Title>
          </StyledModalHeader>
          <Modal.Body>
...
const VesselIcon = styled(VesselListSVG)`
  ...
  height: 25px;
  margin-top: 4px;
  animation: ${p => (!p.$isTitle ? (p.$isRightMenuShrinked ? 'vessel-icon-hidden' : 'vessel-icon-visible') : null)} 0.2s
    ease forwards;
