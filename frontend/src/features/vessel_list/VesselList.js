...
function VesselList({ namespace }) {
  ...
  const { healthcheckTextWarning, previewFilteredVesselsMode, rightMenuIsOpen, vesselListModalIsOpen } = useSelector(
    state => state.global
  )
  const {
    selectedVessel,
      ...
      <Wrapper isFiltering={isFiltering}>
        ...
        <Modal backdrop="static" onClose={() => closeAndResetVesselList()} open={vesselListModalIsOpen} size="full">
          <StyledModalHeader isFull>
            <Modal.Title>
              <Vessel background={COLORS.charcoal} isTitle /> Liste des navires avec VMS
            </Modal.Title>
          </StyledModalHeader>
          <Modal.Body>
...
const Vessel = styled(VesselListSVG)`
  ...
  animation: ${props =>
      !props.isTitle
        ? props.selectedVessel && !props.rightMenuIsOpen
          ? 'vessel-icon-hidden'
          : 'vessel-icon-visible'
        : null}
    0.2s ease forwards;
