...
function VesselsSearch() {
  ...
  const { isFocusedOnVesselSearch, selectedVessel, selectedVesselIdentity, vessels, vesselSidebarIsOpen } = useSelector(
    state => state.vessel,
  )
  ...
  const focusOnVesselSearchInput = useCallback(
    isFocused => {
  ...
  return (
    <>
      ...
      >
        <Flex>
          {isVesselNameShown ? (
            <VesselName focusOnVesselSearchInput={focusOnVesselSearchInput} />
          ) : (
            <Input
