...
export function findMatchingFeature(feature, searchText) {
  return (
    (feature.vesselProperties?.internalReferenceNumber &&
...
export function removeDuplicatedFoundVessels(foundVesselsFromAPI, foundVesselsOnMap) {
  return foundVesselsFromAPI.filter(
    vessel =>
      !(
        (vessel.internalReferenceNumber
          ? foundVesselsOnMap.some(
              vesselFromMap =>
                vesselFromMap.vesselProperties?.internalReferenceNumber === vessel.internalReferenceNumber,
            )
          : false) ||
        (vessel.externalReferenceNumber
          ? foundVesselsOnMap.some(
              vesselFromMap =>
                vesselFromMap.vesselProperties?.externalReferenceNumber === vessel.externalReferenceNumber,
            )
          : false) ||
        (vessel.ircs
