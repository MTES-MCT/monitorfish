...
function ShowFishingActivitiesOnMap({ sidebarIsOpen }) {
  ...
  return (
    <ShowFishingActivitiesOnMapButton
      ...
      onClick={() =>
        fishingActivitiesAreShowedOnMap || fishingActivitiesShowedOnMap?.length
          ? dispatch(hideFishingActivitiesOnMap())
      ...
      title={`${
        fishingActivitiesAreShowedOnMap || fishingActivitiesShowedOnMap?.length ? 'Cacher' : 'Afficher'
