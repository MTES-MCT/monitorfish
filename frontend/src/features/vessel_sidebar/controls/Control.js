...
function Control(props) {
  ...
  return control ? (
    <Wrapper key={index} isLastItem={isLastItem}>
      ...
      <SubValue>{control.controlType}</SubValue>
      <br />
      ...
      <SubFields>
        <SubField>
          ...
          <SubValue>
            {control.controller && control.controller.administration ? (
      ...
      {control.seizure ? (
        <>
          ...
          <Comment>{control.seizureComments}</Comment>
          <br />
        </>
      ) : null}
      <Key width={80}>Observations</Key>
      <SubValue>
        <Comment>{control.postControlComments ? control.postControlComments : <NoValue>-</NoValue>}</Comment>
      </SubValue>
      {control.infractions && control.infractions.length ? (
        <Infractions>
          {control.infractions.map((infraction, index) => <Infraction key={infraction.infractionCategory + index}>
                  <Line>
      ...
      {seeMoreIsOpen ? (
        <More>
          ...
          <SubValue>{control.missionOrder ? 'Oui' : 'Non'}</SubValue>
          <br />
          <Key width={135}>Navire ciblé</Key>
          <SubValue>{control.vesselTargeted ? 'Oui' : 'Non'}</SubValue>
          <br />
          <Key width={135}>Contrôle des engins</Key>
          <SubValue>{control.gearControls && control.gearControls.length ? 'Oui' : 'Non'}</SubValue>
          <br />
          {control.gearControls && control.gearControls.length
            ? control.gearControls.map((gear, index) => <Gear key={gear.gearCode + index}>
          ...
          <SubValue>{control.cooperative === false ? 'Coopératif' : 'Non coopératif'}</SubValue>
        </More>
      ) : null}
