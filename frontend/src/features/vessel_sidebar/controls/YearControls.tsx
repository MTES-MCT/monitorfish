...
export function YearControls({ year, yearControls }: YearControlsProps) {
  ...
  const sortedControls = useMemo(
    () =>
      yearControls.sort((a, b) => new Date(b.controlDatetimeUtc).getTime() - new Date(a.controlDatetimeUtc).getTime()),
    [yearControls]
  )
      ...
      <Row>
        ...
        <YearListContent isOpen={isOpen} name={year.toString()}>
          {sortedControls.map((control, index) => (
            <Control
              key={control.controlDatetimeUtc}
              control={control}
