...
const OneHundredMilesLayer = () => {
    ...
    const vectorSource = new VectorSource({
        ...
        url: (extent) => {
            return (
                ...
                'version=1.1.0&request=GetFeature&typename=monitorfish:'+ LayersEnum.ONE_HUNDRED_MILES +'&' +
    ...
    const vector = new VectorLayer({
        source: vectorSource,
        renderMode: 'image',
        className: Layers.ONE_HUNDRED_MILES,
        style: (feature, _) => {
            return new Style({
                stroke: new Stroke({
                    color: '#05055E',
    ...
    useEffect( () => {
        if(state.layer.layerToShow && state.layer.layerToShow.type === Layers.ONE_HUNDRED_MILES) {
            dispatch({type: 'ADD_LAYER', payload: vector});
        }
    },[state.layer.layerToShow])

    useEffect( () => {
        if(state.layer.layerToHide && state.layer.layerToHide.type === Layers.ONE_HUNDRED_MILES) {
...
export default OneHundredMilesLayer
