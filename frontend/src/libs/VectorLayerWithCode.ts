...
type Options<T extends Feature<Geometry>> = LayerVectorOptions<VectorSource<T>, T> & {
  code: MonitorFishMap.MonitorFishLayer | string
  isClickable?: boolean
  isHoverable?: boolean
}

export class VectorLayerWithCode<T extends Feature<Geometry> = Feature<Geometry>> extends VectorLayer<
  VectorSource<T>,
  T
> {
