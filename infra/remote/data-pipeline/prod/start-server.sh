...
docker run -d -t --rm --network=host --name monitorfish-pipeline-server \
        ...
        prefect server start -d --use-volume --volume-path ~/.prefect/server/pg_data --postgres-port "5433" --expose
