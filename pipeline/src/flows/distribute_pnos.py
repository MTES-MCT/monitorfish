...
def distribute_pnos_flow(
    test_mode: bool,
    is_integration: bool,
    start_hours_ago: int | float,
    end_hours_ago: int | float,
):
    ...
    if generation_needed:
        ...
        # Load pdf documents
        pnos_to_distribute, distribution_needed = load_pno_pdf_documents(rendered_pnos)

        if distribution_needed:
            # Distribute PNOs
            prior_notification_uploads_table = get_table("prior_notification_uploads")
            ...
            messages_to_send = email + sms

            sent_messages = flatten(
                send_pno_message.map(
            ...
            loaded_prior_notification_sent_messages = (
                load_prior_notification_sent_messages(sent_messages)
            )
            ...
            update_logbook_reports_statement = make_update_logbook_reports_statement(
                ...
                start_datetime_utc=start_datetime_utc,
                end_datetime_utc=end_datetime_utc,
                wait_for=[loaded_prior_notification_sent_messages],
            )

            update_manual_pnos_statement = make_manual_prior_notifications_statement(
                pnos_to_update=pnos_to_distribute,
                sent_messages=sent_messages,
                wait_for=[loaded_prior_notification_sent_messages],
            )

            if isinstance(update_logbook_reports_statement, Executable):
                execute_statement(update_logbook_reports_statement)

            if isinstance(update_manual_pnos_statement, Executable):
