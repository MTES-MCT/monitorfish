...
(SELECT DISTINCT ON (r.report_id) -- In rare cases the same PNO with the same data and the same report_id is sent multiple times in messages with different operation numbers
    ...
    ls.last_successful_send_date_utc AS previous_notification_date_utc
...
LEFT JOIN last_sends ls
ON r.operation_type = 'COR'
   AND ls.prior_notification_report_id = r.referenced_report_id
   AND ls.prior_notification_source = 'LOGBOOK'
WHERE
...
(SELECT
    ...
    (value->>'isBeingSent')::BOOLEAN AS is_being_sent,
    'MANUAL' AS source,
    ls.last_successful_send_date_utc IS NOT NULL AS is_correction,
    ls.last_successful_send_date_utc AS previous_notification_date_utc
...
ON p.locode = r.value->>'port'
LEFT JOIN last_sends ls
ON ls.prior_notification_report_id = r.report_id
   AND ls.prior_notification_source = 'MANUAL'
WHERE
    (
        (value->>'isInvalidated') IS NULL
        OR (value->>'isInvalidated')::BOOLEAN IS false
