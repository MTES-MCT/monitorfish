...
def add_activity_dates_table(add_monitorfish_proxy_database):
    client = create_datawarehouse_client()
    print("Creating monitorfish.activity_dates table")
    client.command(
        """
        CREATE TABLE monitorfish.activity_dates
        ENGINE MergeTree()
    ...
    yield
    print("Dropping monitorfish.activity_dates table")
    client.command("DROP TABLE monitorfish.activity_dates")
