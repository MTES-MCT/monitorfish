...
from src.flows.current_segments import (
    compute_current_segments,
    current_segments_flow,
...
def test_extract_current_catches(reset_test_data, current_catches):
    catches = extract_current_catches(number_of_days=90)
...
def test_current_segments_flow(reset_test_data, current_segments):
    state = current_segments_flow(return_state=True)
