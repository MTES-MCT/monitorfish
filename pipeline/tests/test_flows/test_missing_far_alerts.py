...
from sqlalchemy import BOOLEAN, FLOAT, TIMESTAMP, VARCHAR, Column, MetaData, Table, text

from src.db_config import create_engine
from src.exceptions import MonitorfishHealthError
from src.flows.missing_far_alerts import (
    concat,
...
def test_make_positions_at_sea_query():
    ...
    eez_areas_table = Table(
        "eez_areas", meta, Column("geom", Geometry), Column("ISO_SOV1", VARCHAR)
    )

    # Test with all arguments

    query = make_positions_at_sea_query(
    ...
    expected_query_string = (
        ...
        "JOIN eez_areas "
        "ON ST_Intersects(positions.geometry, eez_areas.geom) "
        "\nWHERE "
...
def test_get_vessels_at_sea(
    ...
    vessels_at_sea_1_day = get_vessels_at_sea(positions_at_sea, min_days=1)
    vessels_at_sea_2_days = get_vessels_at_sea(positions_at_sea, min_days=2)
    vessels_at_sea_3_days = get_vessels_at_sea(positions_at_sea, min_days=3)
    vessels_at_sea_4_days = get_vessels_at_sea(positions_at_sea, min_days=4)
...
def test_flow(reset_test_data):
    ...
    with engine.begin() as con:
        con.execute(text("DELETE FROM silenced_alerts"))
