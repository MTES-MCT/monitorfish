...
from src.flows.notify_beacon_malfunctions import (
    ...
    get_templates,
    load_notifications,
    notify_beacon_malfunctions_flow,
    render,
    render_sms,
...
def test_to_malfunctions_to_notify_list():
    ...
    malfunctions_to_notify_list = to_malfunctions_to_notify_list(
...
def test_render(
    ...
    pdf_or_html = render(m=m, templates=templates, output_format=output_format)
...
def test_render_sms(malfunction_to_notify_data, sms_templates, notification_type):
    ...
    sms_text = render_sms(m=m, templates=sms_templates)
...
def test_render_with_null_values(malfunction_to_notify_data_with_nulls, templates):
    ...
    html = render(m=m, templates=templates, output_format="html")
...
def test_create_email(malfunction_to_notify_data, cnsp_logo, notification_type):
    ...
    email_to_send = create_email(html=html, pdf=pdf, m=m)
...
def test_create_sms(malfunction_to_notify_data, notification_type):
    ...
    sms_to_send = create_sms(text=text, m=m)
...
def test_send_beacon_malfunction_message(
    ...
    notifications = send_beacon_malfunction_message(
...
def test_load_notifications(reset_test_data, expected_notifications):
    ...
    load_notifications(expected_notifications)
...
def test_flow(reset_test_data):
    ...
    # removed by the prefect filter_results task.
    ...
    # Now all beacon malfunctions have been notified, there should be no notifications
    # to notify
    state = notify_beacon_malfunctions_flow(
        test_mode=False, is_integration=True, return_state=True
    )
    malfunctions_to_notify = extract_malfunctions_to_notify()
    assert len(malfunctions_to_notify) == 0
