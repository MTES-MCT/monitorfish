...
french_vessels_data = {
    ...
    "proprietor_address": [
        "1 Place des poissecailles FRANCE",
        "12 Avenue du Paradis 56200 CHEPAOU",
    ],
    "proprietor_nationality": ["FRANCE", "FRANCE"],
...
eu_vessels_data = {
    ...
    "proprietor_name": ["Fishing Co.", None],
    "proprietor_address": [
        "Fishing Co. 9874 Brooklyn USA",
        "Tuna Co. 51 Flying Fish Avenue AZ8855",
    ],
    "proprietor_nationality": [None, None],
...
concat_merged_data = {
    ...
    "proprietor_nationality": ["FRANCE", "FRANCE", None, None, None, None],
...
@patch("src.flows.vessels.extract")
def test_extract_eu_vessels(mock_extract):
    mock_extract.side_effect = mock_extract_side_effect
    query = extract_eu_vessels()
...
def test_extract_nav_licences(mock_extract):
    mock_extract.side_effect = mock_extract_side_effect
    query = extract_french_vessels_navigation_licences()
    assert isinstance(query, sqlalchemy.sql.elements.TextClause)
...
def test_extract_vessels_logbook_equipement(mock_extract):
    mock_extract.side_effect = mock_extract_side_effect
    query = extract_vessels_logbook_equipement()
    assert isinstance(query, sqlalchemy.sql.elements.TextClause)
...
def test_extract_vessels_operators(mock_extract):
    mock_extract.side_effect = mock_extract_side_effect
    query = extract_vessels_operators()
    assert isinstance(query, sqlalchemy.sql.elements.TextClause)
...
def test_add_unknown_vessel(cleaned_vessels):
    ...
    with pytest.raises(AssertionError):
        add_unknown_vessel(vessels)


def test_load_vessels(reset_test_data, cleaned_vessels):
    load_vessels(cleaned_vessels)
