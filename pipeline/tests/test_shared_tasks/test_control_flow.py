...
from src.shared_tasks.control_flow import filter_results, flatten, str_to_path
...
@task
def another_list(i: int) -> list:
    return [i + j for j in [0, 1, 2]]
...
def filter_flow():
    ...
    list_of_lists = another_list.map(added_li)
    flattened_li = flatten(list_of_lists)
    reduced_li = reduce(flattened_li)
    return (
        li,
        funky_li,
        filtered_li,
