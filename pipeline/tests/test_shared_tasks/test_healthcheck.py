...
def test_get_monitorfish_healthcheck():
    with Mocker(json_encoder=json.encoder.JSONEncoder) as requests_mocker:
        ...
        health = get_monitorfish_healthcheck()
...
def test_assert_last_positions_flow_health_with_default_parameters(healthcheck, utcnow):
    assert_last_positions_flow_health(healthcheck=healthcheck, utcnow=utcnow)
...
def test_assert_logbook_health_with_default_parameters(healthcheck, utcnow):
    assert_logbook_health(healthcheck=healthcheck, utcnow=utcnow)
